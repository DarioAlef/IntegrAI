...
async def webhook(request: Request):
    ...
    authenticated_user = None  # Usuário do banco de dados associado ao remetente.
    ...
    if "data" in data and "message" in data["data"] and "key" in data["data"]:
        ...
        message = await processar_texto(msg_data)
        ...
        await messenger.enviar_mensagem("Infelizmente não pudemos capturar sua mensagem por problemas técnicos. Tente novamente mais tarde.")
    ...
    if valid_user_message(message, from_me, authenticated_user):
        await store_message(authenticated_user, 'user', message, True)  

    # ==============================================================================================
    # 5. Iniciar o serviço.
    # ==============================================================================================
        if not await command_handler(authenticated_user, message, sender_number):
